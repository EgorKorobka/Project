...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label_theme = CTkOptionMenu(self.frame, values=['', ''],command=self.change_theme)
        ...
        self.message_input = CTkEntry(self.main_frame, placeholder_text='')
        self.message_input.grid(row=0,column=0,sticky='ew', padx=5,pady=5)
    ...
    def animate_menu(self):
        if self.is_show_menu:
            ...
            if self.frame_width > 0:
                self.frame -= self.menu_show_speed
                self.frame.configure(width=self.frame_width)
                self.btn.configure(text='☢')
                self.after(10,self.animate_menu)
